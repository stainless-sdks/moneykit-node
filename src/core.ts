...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from 'moneykit'`:
   * - `import 'moneykit/shims/node'` (if you're running on Node)
   * - `import 'moneykit/shims/web'` (otherwise)
   */
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new MoneykitError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new MoneykitError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new MoneykitError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process.env['DEBUG'] === 'true') {
    console.log(`Moneykit:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new MoneykitError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
